...
Going along with this I did correlation calculations between the delay data and nextDayCFU:
```{r, eval=FALSE}

meta <- read.csv('~/Desktop/mothur/abxD01/rf/abxD01.final.an.unique_list.0.03.subsample.0.03.pick.shared.rf.delay.regression.logtrans.filter16mintot.noUntr.csv',header=T)
#meta<-meta[1:96] #change based on number of OTUs above .05%, then add 2 for first two cols
meta$nextDayCFU <- 10^(meta$nextDayCFU)
c<-1
otu <- c()
cor.spear <- c()
pval.spear <- c()
...
write.table(results[1:dim(results)[1],], file="~/Desktop/mothur/abxD01/correlation/abxD01.final.an.unique_list.0.03.subsample.filter16mintotal.shared.correl.delay.noUntr.results.txt", sep="\t", row.names=FALSE)
