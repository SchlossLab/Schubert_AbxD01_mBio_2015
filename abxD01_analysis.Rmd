...
The code for this is the script "stacked.barcharts.r" which contains the stackedbarcharts() function in the paper's home Github directory. Opening this file, the parameters I changed in the top were:
```{r, eval=FALSE}
setwd("/Users/alyxschubert/Documents/Github/abxD01/")

abx<-c("5 mg/ml", "0.5 mg/ml", "0.1 mg/ml")
stackedbarcharts(file = "./Figure 3/abxD01.final.tx.2.subsample.allceftitr.forlogscale.csv", fileIDS = "./Figure 3/allceftitr_tx2_barchart_ids.csv", graphLabels = abx, sortbyphyl = TRUE, graphbyphyl = TRUE, excludeGroups = "1untrCef")

abx<-c("5 mg/ml", "0.5 mg/ml", "0.1 mg/ml")
stackedbarcharts(file = "./Figure 3/abxD01.final.tx.2.subsample.allstreptitr.forlogscale.csv", fileIDS = "./Figure 3/allstreptitr_tx2_barchart_ids.csv", graphLabels = abx, sortbyphyl = TRUE, graphbyphyl = TRUE, excludeGroups = "1untrStrep")

abx<-c("5 mg/ml", "0.5 mg/ml", "0.1 mg/ml")
stackedbarcharts(file = "./Figure 3/abxD01.final.tx.2.subsample.allvanctitr.forlogscale.csv", fileIDS = "./Figure 3/allvanctitr_tx2_barchart_ids.csv", graphLabels = abx, sortbyphyl = TRUE, graphbyphyl = TRUE, excludeGroups = "1untrVanc")
...
strep_avgCD<-format(avgCD, scientific=TRUE, digits=2)
file$nextDayCFU <- log10(file$nextDayCFU+1)
#kruskal.test(file$nextDayCFU ~ file$expgroup)
# p < 0.0001
#strep_CFU_wilcox <- pairwise.wilcox.test(file$nextDayCFU, file$expgroup, p.adj="BH")
# all significantly different
strep.aov<-aov(file$nextDayCFU ~ file$expgroup)
anova(strep.aov)
#TukeyHSD(strep.aov)
strep_ttest <- pairwise.t.test(file$nextDayCFU, file$expgroup, p.adjust="BH")
print(strep_ttest)
# all significantly different
...
avgCD<-tapply(file$nextDayCFU, file$expgroup, mean)
cef_avgCD<-format(avgCD, scientific=TRUE, digits=2)
file$nextDayCFU <- log10(file$nextDayCFU+1)
#kruskal.test(file$nextDayCFU ~ file$expgroup)
# p = 0.003
#cef_CFU_wilcox <- pairwise.wilcox.test(file$nextDayCFU, file$expgroup, p.adj="BH")
# mid and low dose not different, but high different than mid and low
cef.aov<-aov(file$nextDayCFU ~ file$expgroup)
anova(cef.aov)
#TukeyHSD(cef.aov)
cef_ttest <- pairwise.t.test(file$nextDayCFU, file$expgroup, p.adjust="BH")
print(cef_ttest)
# all significantly different
...
avgCD<-tapply(file$nextDayCFU, file$expgroup, mean)
vanc_avgCD<-format(avgCD, scientific=TRUE, digits=2)
file$nextDayCFU <- log10(file$nextDayCFU+1)
#kruskal.test(file$nextDayCFU ~ file$expgroup)
# p = 0.0016
#vanc_CFU_wilcox <- pairwise.wilcox.test(file$nextDayCFU, file$expgroup, p.adj="BH")
# mid and low dose not different, but high different than mid and low
vanc.aov<-aov(file$nextDayCFU ~ file$expgroup)
anova(vanc.aov)
#TukeyHSD(vanc.aov)
vanc_ttest <- pairwise.t.test(file$nextDayCFU, file$expgroup, p.adjust="BH")
print(vanc_ttest)
# mid and low dose not different, but high different than mid and low
...
For comparisons between family-level phylotypes:
Performed inside abxD01.barcharts.xOTU.test.r
```{r, eval=FALSE}
file<-read.csv("~/Documents/Github/abxD01/Figure 3/abxD01.final.tx.2.subsample.allstreptitr.forlogscale.csv", header=T)
file<-file[file$expgroup!="1untrStrep",]
...
avgCD<-tapply(file$nextDayCFU, file$expgroup, mean)
metro_avgCD<-format(avgCD, scientific=TRUE, digits=2)
file$nextDayCFU <- log10(file$nextDayCFU+1)
#kruskal.test(file$nextDayCFU ~ file$expgroup)
# p < 0.0001
#strep_CFU_wilcox <- pairwise.wilcox.test(file$nextDayCFU, file$expgroup, p.adj="BH")
# all significantly different
metro.aov<-aov(file$nextDayCFU ~ file$expgroup)
anova(metro.aov)
#TukeyHSD(strep.aov)
metro_ttest <- t.test(file$nextDayCFU~file$expgroup)
print(metro_ttest)
...
Then in r: 
```{r, eval=FALSE}
topdose_logtrans<-read.csv("~/Desktop/mothur/abxD01/rf/abxD01.final.an.unique_list.0.03.subsample.0.03.shared.rf.topdose2.regression.logtrans.filter16mintot.filter16mintot.csv", header=T)
